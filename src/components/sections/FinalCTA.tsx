...
const FinalCTA = () => {
  ...
  // Single pulse on load then stop
  useState(() => {
    if (isInView && !hasPulsed) {
      setTimeout(() => setHasPulsed(true), 1500);
    }
  });
            ...
            <SciFiPanel label="Access Request">
              <form onSubmit={handleSubmit} className="p-5">
                <input
                  ...
                  required
                  className="w-full px-3 py-2.5 mb-4 bg-background border border-primary/30 text-sm font-mono placeholder:text-muted-foreground focus:outline-none focus:border-primary/60 transition-colors duration-300"
                />
                <motion.div
                  animate={!hasPulsed ? { boxShadow: ['0 0 0 0 hsl(var(--primary) / 0)', '0 0 20px 4px hsl(var(--primary) / 0.3)', '0 0 0 0 hsl(var(--primary) / 0)'] } : {}}
